...
| A11Y-008 | 3.3.1 / 3.3.2 / 3.3.3 / 4.1.3 forms | `/` contact process; `/newsletter/` signup process; both `/thanks/` pages | Added required cues, linked summaries, persistent field errors, invalid/describedby state, progress status, and confirmation checks. | 2026-08-24 | Codex, Chrome/Playwright | **Pass:** complete empty-to-valid-to-confirmation processes passed locally; live empty/error states passed. A real Netlify delivery and screen-reader retest remain pending. |
...
| A11Y-014 | Third-party accessibility | Netlify Forms, Google Maps, Compass/RSR/listing handoffs | Obtain evidence; test deployed flow; escalate vendor defects; preserve direct email/phone fallback. | 2026-08-24 | Codex production-safe test; independent tester not yet assigned | **Open:** deployed form error states passed, but real Netlify delivery, vendor evidence, external interfaces, and assistive-technology testing remain open. |

## Production retest entries

| Date | Release | Scope/environment | Tester | Result |
|---|---|---|---|---|
| 2026-08-24 | `085d265` | All 27 canonical production URLs; Chrome/Playwright; axe-core 4.13.0; 320 CSS-pixel reflow | Codex | **Pass:** zero axe A/AA violations and no page-level horizontal overflow. |
| 2026-08-24 | `085d265` | Live home-page mobile navigation, Escape/focus return, skip link, reduced motion, focus indicator, and both empty form-error states | Codex | **Pass:** safe interaction checks succeeded; no real records were submitted. |
| 2026-08-24 | `085d265` | Live `/legal/` accessibility statement | Codex | **Pass:** WCAG 2.2 AA target, Julia’s email, 818-859-0762, two-business-day response target, and known limitations are present. |

Continue adding dated entries after independent testing. Include the production URL, browser/assistive technology, exact task, tester, and outcome. Do not convert an open manual/PDF item to pass based solely on axe or Lighthouse.
